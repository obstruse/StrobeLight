/* 
---------------------------------------------
A Modified Version of Adafruit_DotStar v1.1.5
  - for ESP32 -
---------------------------------------------
*/
...
Adafruit_DotStar::Adafruit_DotStar(uint16_t n, uint8_t o)
    : numLEDs(n), dataPin(USE_HW_SPI), brightness(0), pixels(NULL), SPIbuf(NULL),
...
Adafruit_DotStar::Adafruit_DotStar(uint16_t n, uint8_t data, uint8_t clock,
                                   uint8_t o)
    : dataPin(data), clockPin(clock), brightness(0), pixels(NULL), SPIbuf(NULL),
      rOffset(o & 3), gOffset((o >> 2) & 3), bOffset((o >> 4) & 3) {
  updateLength(n);
}
...
void Adafruit_DotStar::updateLength(uint16_t n) {
  ...
  if ((pixels = (uint8_t *)malloc(bytes))) {
    ...
    Serial.println("pixel malloc failed");
    numLEDs = 0;
  }

  // SPI output buffer
  free(SPIbuf);
  // hmm... don't see anything to support MONO, so why allocate?
  // COLOR: four bytes per pixel + four bytes frame start + ((numLEDs + 15) / 16) bytes frame end
  SPIbufBytes = (n * 4) + 4 + ((n + 15) / 16);
  if ((SPIbuf = (uint8_t *)malloc(SPIbufBytes))) {
    memset(SPIbuf,0xFF,SPIbufBytes);
    memset(SPIbuf,0x00,4);
  } else {
    Serial.println("SPIbuf malloc failed");
    numLEDs = 0;
  }
}
...
#define spi_out(n) (void)SPI.write(n) ///< Call hardware SPI function
...
#elif defined(ESP32)
    if (n & 0x80) {
      GPIO.out_w1ts = (1 << dataPin);
      //__asm__ volatile("nop \n nop");
    } else {
      //__asm__ volatile("nop \n nop");
      GPIO.out_w1tc = (1 << dataPin);
    }

    GPIO.out_w1ts = (1 << clockPin);
    GPIO.out_w1tc = (1 << clockPin);
...
void Adafruit_DotStar::show(void) {
  ...
  uint8_t *SPIptr = SPIbuf;            // -> SPI buffer
  ...
  if (dataPin == USE_HW_SPI) {
    ...
    for (i = 0; i < ((numLEDs + 15) / 16); i++)   // end frame
      spi_out(0xFF);      

#elif defined(ESP32) || defined(ESP8266)
    SPIptr+=4;         // frame start already in buffer
    if (brightness) {  // Scale pixel brightness on output
      do {             // For each pixel...
        SPIptr++;      // Pixel start already in buffer
        for (i = 0; i < 3; i++)
          *SPIptr++ = (*ptr++ * b16) >> 8; // Scale, write RGB
      } while (--n);
    ...
    // end frame included in buffer
    SPI.writeBytes(SPIbuf,SPIbufBytes);
